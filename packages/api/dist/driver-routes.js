...
export const driverLogin = async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = jwt.sign({ id: driver.id, email: driver.email, role: 'driver', name: driver.name }, JWT_SECRET, { expiresIn: '30d' });
        res.json({
            token,
            driver: {
                id: driver.id,
...
export const driverSignup = async (req, res) => {
    try {
        ...
        const [newDriver] = await db
            ...
            .returning();
        // Generate JWT token
        const token = jwt.sign({ id: newDriver.id, email: newDriver.email, role: 'driver', name: newDriver.name }, JWT_SECRET, { expiresIn: '30d' });
        res.json({
            token,
            driver: {
                id: newDriver.id,
...
// Driver Me
export const driverMe = async (req, res) => {
    try {
...
export const getAvailableJobs = async (req, res) => {
    try {
        const availableJobs = await db
            .select()
            .from(orders)
            .where(and(or(eq(orders.status, 'pending'), eq(orders.status, 'paid')), isNull(orders.assignedDriverId)))
