...
// GET /admin/auth/me
export const adminMe = async (req: AuthRequest, res: Response) => {
  try {
...
export const getStatsOverview = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const driverStats = driverStatsResult.rows[0] as {
      total: string;
      pending: string;
      approved: string;
      blocked: string;
    ...
    const customerStats = customerStatsResult.rows[0] as { total: string };
    ...
    const orderStats = orderStatsResult.rows[0] as {
      total: string;
      today: string;
      this_week: string;
      this_month: string;
