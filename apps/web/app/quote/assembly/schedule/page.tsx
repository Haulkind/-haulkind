...
export default function AssemblySchedulePage() {
  ...
  const handleContinue = () => {
    if (!street.trim() || !city.trim() || !usState.trim() || !zip || !date || !time || !floor || !boxStatus) {
      ...
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between text-sm text-gray-500 mb-2">
            ...
            <span className="font-medium text-orange-600">Date & Location</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-orange-500 h-2 rounded-full transition-all" style={{ width: '50%' }} />
          </div>
        </div>

        <h1 className="text-3xl font-bold text-gray-900 mb-2">When and where?</h1>
        <p className="text-gray-600 mb-8">Tell us when and where you need the assembly done.</p>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 space-y-5">
          ...
          <div className="grid grid-cols-10 gap-3">
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                placeholder="PA"
                maxLength={2}
                className="w-full h-11 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent uppercase"
              />
            </div>
          ...
          {/* Where bought */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Where did you buy the furniture? (optional)</label>
            <select
              value={store}
            ...
            {store === 'other' && (
              <input
                ...
                className="w-full h-10 px-3 py-2 mt-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              />
            )}
          </div>
          ...
          {/* Box status */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Are the items still in the box? *</label>
            <div className="space-y-2">
              <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition ${boxStatus === 'unopened' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'}`}>
                <input type="radio" name="boxStatus" value="unopened" checked={boxStatus === 'unopened'} onChange={(e) => setBoxStatus(e.target.value)} className="text-orange-600" />
                <span className="text-sm">Yes, unopened</span>
              </label>
              <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition ${boxStatus === 'partial' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'}`}>
                <input type="radio" name="boxStatus" value="partial" checked={boxStatus === 'partial'} onChange={(e) => setBoxStatus(e.target.value)} className="text-orange-600" />
                <span className="text-sm">Partially assembled — need help finishing</span>
              </label>
              <label className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition ${boxStatus === 'reassembly' ? 'border-orange-500 bg-orange-50' : 'border-gray-200 hover:border-gray-300'}`}>
                <input type="radio" name="boxStatus" value="reassembly" checked={boxStatus === 'reassembly'} onChange={(e) => setBoxStatus(e.target.value)} className="text-orange-600" />
                <span className="text-sm">Need disassembly and reassembly</span>
              </label>
            </div>
          </div>
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            ...
            className="w-3/5 h-12 bg-orange-500 text-white rounded-lg font-semibold hover:bg-orange-600 transition"
          >
            Continue
          </button>
        </div>
